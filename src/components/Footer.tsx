      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 pt-14 pb-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div className="ft-col lg:col-span-1">
          ...
          <p className="text-white/55 text-sm leading-relaxed mb-6">
            Your trusted partner for SME IPO advisory, pre-IPO fundraising, and successful capital market listings.
          </p>
          <div className="flex flex-col gap-2.5">
            {[{ label: "SEBI Registered", sub: "Category I Merchant Banker" }, { label: "ICAI Member", sub: "Chartered Accountants" }].map((b) => (
              <div key={b.label} className="flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-xs" style={{ border: "1px solid rgba(245,158,11,0.2)", background: "rgba(245,158,11,0.05)" }}>
                ...
                <div><p className="text-white font-bold">{b.label}</p><p className="text-white/40">{b.sub}</p></div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="ft-col">
          ...
          <address className="not-italic text-sm text-white/60 space-y-3 mb-6">
            ...
            <a href="tel:+91XXXXXXXXXX" className="flex items-start gap-2.5 hover:text-brand-gold transition-colors duration-200">
          ...
          <Link href="/contact-us"
            className="inline-flex items-center justify-center px-6 py-2.5 rounded-xl font-bold text-brand-navy-dark text-sm cursor-pointer hover:opacity-90 transition-opacity"
